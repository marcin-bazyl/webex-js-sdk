...
const hackRemoteCandidates = (sdp) => {
  if (window.hack) {
    const sdpLines = sdp.split(SDP.CARRIAGE_RETURN);

    const {
      disableUdp, type, ip, port, changeAll
    } = window.hack;

    console.log('marcin: hacking remote candidates....');
    console.log('marcin: disableUdp=', disableUdp);
    console.log('marcin: type=', type);
    console.log('marcin: ip=', ip);
    console.log('marcin: port=', port);
    console.log('marcin: changeAll=', changeAll);
    // window.hack = {disableUdp:true, type:'ssltcp', ip:'10.47.200.45', port:'443'}
    // SSLTCP

    let final;

    if (changeAll) {
      final = sdpLines.map((line) => {
        if (line.search(/^a=candidate:[01] 1 UDP/) !== -1) {
          // const priority = 116 * (2 << 7) + (localPriority * (2 << 7)) + 255;
          return line.replace(/^a=candidate:([\d]) 1 UDP (\d+) (\d+.\d+.\d+.\d+) (\d+) typ host/g, `a=candidate:$1 1 UDP $2 ${ip} $4 typ host`);
        } if (line.search(/^a=candidate:2 1 TCP/) !== -1) {
          return line.replace(/^a=candidate:([\d]) 1 TCP (\d+) (\d+.\d+.\d+.\d+) (\d+) typ host tcptype passive/g, `a=candidate:$1 1 ${type} $2 ${ip} ${port} typ host tcptype passive`);
        }
      ...
      const filtered = disableUdp ? sdpLines.filter((line) => line.search(/a=candidate:[01] 1 UDP/) === -1) : sdpLines;

      final = filtered.map((line) => line.replace(/a=candidate:2 1 TCP.*/gi, `a=candidate:2 1 ${type} 1962934271 ${ip} ${port} typ host tcptype passive`));
    }

    const mungedSdp = final.join(SDP.CARRIAGE_RETURN);

    return mungedSdp;
  }
  console.log('marcin: not doing any hacks');

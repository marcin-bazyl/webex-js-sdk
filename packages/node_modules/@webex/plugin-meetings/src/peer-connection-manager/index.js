...
const isSdpInvalid = (sdp) => {
  ...
  for (const mediaLine of parsedSdp.media) {
    if (!mediaLine.candidates || mediaLine.candidates?.length === 0) {
      ...
      console.log(`marcin: Ice gathering never completed: ${mediaLine.mid}`, mediaLine.candidates);
    ...
    if (SDP.BAD_MEDIA_PORTS.includes(mediaLine.port)) {
      ...
      console.log('marcin: found invalid port');
    ...
    if (!mediaLine.icePwd || !mediaLine.iceUfrag) {
      ...
      console.log('marcin: ice ufrag and password not found');
  ...
  new Promise((resolve, reject) => {
    ...
    const doneGatheringIceCandidate = () => {
      ...
      if (invalidSdpPresent) {
        console.log('marcin: SDP not valid after waiting');
    ...
    peerConnection.onIceGatheringStateChange = () => {
      if (peerConnection.iceGatheringState === COMPLETE) {
        console.log('marcin: Ice state changed to COMPLETE');
        doneGatheringIceCandidate(peerConnection);
      }
      if (peerConnection.iceGatheringState === GATHERING) {
        console.log('marcin: Ice state changed to gathering');
    ...
    peerConnection.onicecandidate = (evt) => {
      if (evt.candidate === null) {
        console.log('marcin: null candidate');
        doneGatheringIceCandidate(peerConnection);
      }
      else {
        console.log(`marcin --> Candidate ${evt.candidate?.type} ${evt.candidate?.protocol} ${evt.candidate?.address}:${evt.candidate?.port}`);
    ...
    peerConnection.onicecandidateerror = (event) => {
      console.log('marcin: onicecandidateerror');

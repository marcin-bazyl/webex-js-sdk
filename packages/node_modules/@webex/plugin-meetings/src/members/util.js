...
MembersUtil.generateMuteMemberOptions = (mute, status, requestToUnmute, locusUrl) => ({
  memberId: mute,
  muted: status,
  requestedToUnmute: requestToUnmute,
  locusUrl
});

MembersUtil.getMuteMemberRequestParams = (options) => {
  const body = options.requestedToUnmute ? {
    audio: {
      requestedToUnmute: options.requestedToUnmute
    }
  } : {
    audio: {
      muted: options.muted

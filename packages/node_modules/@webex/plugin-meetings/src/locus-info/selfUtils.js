...
SelfUtils.parse = (self, deviceId) => {
  if (self) {
    ...
    return {
      ...
      requestedToUnmute: SelfUtils.getRequestedToUnmute(self),
...
SelfUtils.getSelves = (oldSelf, newSelf, deviceId) => {
  ...
  updates.isRequestedToUnmuteReceived = SelfUtils.requestedToUnmuteReceived(previous, current);
  updates.isLocalAudioUnmuteRequiredChanged = SelfUtils.localAudioUnmuteRequiredChanged(previous, current);
  ...
  // todo: updates.mediaStatusChanged =
...
SelfUtils.getRequestedToUnmute = (self) => {
  if (!self || !self.controls || !self.controls.audio) {
    return null;
  }

  return !!self.controls.audio.requestedToUnmute;
};
...
SelfUtils.getLocalAudioUnmuteRequired = (self) => {
  if (!self || !self.controls || !self.controls.audio) {
    return null;
  }

  return !!self.controls.audio.localAudioUnmuteRequired;
};
...
SelfUtils.mutedByOthersChanged = (oldSelf = {}, changedSelf) => {
  ...
  if (!oldSelf && !changedSelf.remoteMuted) {
...
SelfUtils.requestedToUnmuteReceived = (oldSelf = {}, changedSelf) => {
  if (!changedSelf) {
    throw new ParameterError('New self must be defined to determine if self received request to unmute.');
  }

  return changedSelf.requestedToUnmute && !oldSelf.requestedToUnmute;
};


SelfUtils.localAudioUnmuteRequiredChanged = (oldSelf = {}, changedSelf) => {
  if (!changedSelf) {

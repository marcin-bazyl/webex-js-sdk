...
import {RemoteMediaManager, Event as RemoteMediaManagerEvent, Configuration as RemoteMediaManagerConfig} from '../multistream/remoteMediaManager';
  ...
  setupMediaConnectionListeners = () => {
    ...
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.ACTIVE_SPEAKERS_CHANGED,
      (msg) => {
        Trigger.trigger(
          ...
          {
            seqNum: msg.seqNum, // todo: maybe keep track of seqNum and only emit if it's bigger then the last one
    ...
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.VIDEO_SOURCES_COUNT_CHANGED,
      (numTotalSources, numLiveSources) => {
        console.log(`marcin2: got VIDEO: ${numLiveSources} / ${numTotalSources}`);
        Trigger.trigger(
          this,
          {
            file: 'meeting/index',
    ...
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.AUDIO_SOURCES_COUNT_CHANGED,
      (numTotalSources, numLiveSources) => {
        console.log(`marcin2: got AUDIO: ${numLiveSources} / ${numTotalSources}`);
        Trigger.trigger(
          this,
          {
            file: 'meeting/index',
    ...
    return MeetingUtil.validateOptions(options)
      .then(() => {
        ...
        if (this.isMultistream) {
          // todo: probably it would be better to create it in ctor and have clients register for events directly with it rather than with the meeting object
          ...
          return this.remoteMediaManager.start()
            .then(() => this.mediaProperties.webrtcMediaConnection.initiateOffer());
        }

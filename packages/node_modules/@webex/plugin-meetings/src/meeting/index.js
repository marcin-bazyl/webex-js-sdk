...
import {ConnectionState, MediaConnection, Event, RemoteTrackType} from '@webex/webrtc-media';
...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
   ...
   * event to developers
   * @param {Object} mc The media connection object
   * @returns {undefined}
   * @public
   * @memberof Meeting
   */
  setRemoteStream(mc) {
    console.log('marcin: registering for REMOTE_TRACK_ADDED');
    // eslint-disable-next-line no-param-reassign
    mc.on(Event.REMOTE_TRACK_ADDED, (event) => {
      LoggerProxy.logger.log(`Meeting:index#waitForRemoteStream --> ontrack event received for peerConnection: ${event}`);
      console.log(`marcin: Event.REMOTE_TRACK_ADDED: ${event}`);
      ...
      // todo: this.statsAnalyzer.startAnalyzer(this.mediaProperties.peerConnection);
  ...
  closePeerConnections() {
    return this.mediaProperties.peerConnection.close();
  }
    ...
    return MeetingUtil.validateOptions(options)
      .then(() => {
        ...
        const mc = new MediaConnection({
          ...
          send: {
            audio: this.mediaProperties.audioTrack,
            video: this.mediaProperties.videoTrack
          },
          receive: {
            audio: true,
            video: true,
        ...
        this.mediaProperties.setMediaPeerConnection(mc); // (MediaUtil.createPeerConnection());
        this.setMercuryListener();
        // PeerConnectionManager.setPeerConnectionEvents(this); // todo

        // todo: remote participant starts screen share, then stops, then connection gets dropped by the server due to inactivity
        console.log('marcin: registering for ROAP_MESSAGE_TO_SEND');
        mc.on(Event.ROAP_MESSAGE_TO_SEND, (event) => {
      ...
      // })
        ...
        // const {peerConnection} = this.mediaProperties;
          ...
          new Promise((resolve, reject) => {
            const connState = mc.getConnectionState();

            console.log(`marcin: mc.getConnectionState(): ${connState}`);
  ...
  updateAudio(options) {
    ...
    return MeetingUtil.validateOptions({sendAudio, localStream: stream})
      .then(() => {
        ...
        // todo: for now we only do 1 thing - either update receive or send options, we need
        // to add api in webrtc-core to allow updating both together at the same time
        if (previousMediaDirection.receiveTrack !== options.receiveAudio) {
          return this.mediaProperties.webrtcMediaConnection.updateReceiveOptions({
        ...
        return this.mediaProperties.webrtcMediaConnection.updateSendOptions({audio: track});
  ...
  updateVideo(options) {
    ...
    return MeetingUtil.validateOptions({sendVideo, localStream: stream})
      .then(() => {
        // todo: for now we only do 1 thing - either update receive or send options, we need
        // to add api in webrtc-core to allow updating both together at the same time
        if (this.mediaProperties.mediaDirection.receiveVideo !== options.receiveVideo) {
          return this.mediaProperties.webrtcMediaConnection.updateReceiveOptions({
        ...
        return this.mediaProperties.webrtcMediaConnection.updateSendOptions({video: track});
      }
  ...
  updateShare(options) {
    ...
    return MeetingUtil.validateOptions({sendShare, localShare: stream})
      ...
      .then((startShare) => {
        // todo: for now we only do 1 thing - either update receive or send options, we need
        // to add api in webrtc-core to allow updating both together at the same time
        if (this.mediaProperties.mediaDirection.receiveShare !== options.receiveShare) {
          return this.mediaProperties.webrtcMediaConnection.updateReceiveOptions({
            audio: this.mediaProperties.mediaDirection.receiveAudio,
            video: this.mediaProperties.mediaDirection.receiveVideo,
        ...
        return this.mediaProperties.webrtcMediaConnection.updateSendOptions({screenShareVideo: track})
        // MeetingUtil.updateTransceiver({
          //   type: 'video',
          //   sendTrack: sendShare,

...
// todo: not sure why I need this, but seems like I'm getting duplicate notifications for same remote track
// that then cause "Failed to execute 'createEncodedStreams' on 'RTCRtpReceiver': Encoded video streams already created" DOMException
let mainTransformSetupDone = false;
let shareTransformSetupDone = false;
...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  constructor(attrs, options) {
     ...
     * @memberof Meeting
     */
    this.preferTranscoding = undefined;
    /**
     * @instance
     * @type {Object}
     * @readonly
  ...
  closePeerConnections() {
    if (this.mediaProperties.webrtcMediaConnection) {
      return this.mediaProperties.webrtcMediaConnection.close();
    }
  ...
  join(options = {}) {
    ...
    this.preferTranscoding = options.preferTranscoding;
    console.log(`marcin: stored preferTranscoding=${this.preferTranscoding}`);
  ...
  setupMediaConnectionCallbacks = () => {
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.ROAP_MESSAGE_TO_SEND, (event) => {
      const LOG_HEADER = 'Meeting:index#setupMediaConnectionCallbacks.ROAP_MESSAGE_TO_SEND -->';
      ...
      switch (event.roapMessage.messageType) {
        case 'OK':
          logRequest(this.roap.sendRoapOK({
            ...
            header: `${LOG_HEADER} Send Roap OK Request.`,
            success: `${LOG_HEADER} Successfully send roap OK request`,
            failure: `${LOG_HEADER} Error joining the call on send roap OK request, `
          });
          break;
        ...
        case 'OFFER':
          logRequest(this.roap
            .sendRoapMediaRequest({
              ...
              roapSeq: event.roapMessage.seq - 1, // temp hack because sendRoapMediaRequest adds 1
              meeting: this, // or can pass meeting ID
              reconnect: this.reconnectionManager.isReconnectInProgress()
            }), {
            header: `${LOG_HEADER} Send Roap Offer Request.`,
            success: `${LOG_HEADER} Successfully send roap offer request`,
            failure: `${LOG_HEADER} Error joining the call on send roap offer request, `
          });
          break;
        ...
        case 'ANSWER':
          logRequest(this.roap.sendRoapAnswer({
            ...
            header: `${LOG_HEADER} Send Roap Answer Request.`,
            success: `${LOG_HEADER} Successfully send roap answer request`,
            failure: `${LOG_HEADER} Error joining the call on send roap answer request, `
          });
          break;
        ...
        default:
          console.log('marcin: todo - not implemented:', event.roapMessage.messageType);
          break;
      }
    });
    ...
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.REMOTE_TRACK_ADDED, (event) => {
      LoggerProxy.logger.log(`Meeting:index#setupRemoteTrackListener --> ontrack event received for peerConnection: ${JSON.stringify(event)}`);

      console.log(`marcin: Event.REMOTE_TRACK_ADDED: ${JSON.stringify(event)}`);
      ...
      switch (event.type) {
        ...
        case MC.RemoteTrackType.VIDEO:
          ...
          this.mediaProperties.setRemoteVideoTrack(event.track); // todo support multiple tracks
          break;
        case MC.RemoteTrackType.SCREENSHARE_VIDEO:
          ...
          break;
        default: {
          LoggerProxy.logger.log('Meeting:index#waitForRemoteStream --> unexpected track');
        }
      }

      // start stats here the stats are coming null if you dont receive streams

      // todo: this.statsAnalyzer.startAnalyzer(mediaProperties.peerConnection);
      ...
      if (eventType && mediaTrack) {
        Trigger.trigger(
          ...
          {
            type: eventType,
            stream: event.stream || MediaUtil.createMediaStream([mediaTrack]),
            slotId: event.slotId
          }
        );
      }

      if (!mainTransformSetupDone &&
          (eventType === EVENT_TYPES.REMOTE_VIDEO) &&
          (this.config.experimental.shareReceiverTransform?.worker) &&
          (event.slotId === 0)) {
        const transceiver = this.mediaProperties.webrtcMediaConnection.multistreamConnection.pc.getTransceivers()
          .find((tr) => (tr.receiver.track.id === event.track.id));

        const receiverStreams = transceiver.receiver.createEncodedStreams();

        const {readable, writable} = receiverStreams;

        this.config.experimental.shareReceiverTransform.worker.postMessage({
          operation: 'decode main',
          readable,
          writable
        }, [readable, writable]);

        console.log('marcin: setting up shareReceiverTransform for remote main video');
        mainTransformSetupDone = true;
      }

      if (!shareTransformSetupDone &&
          (eventType === EVENT_TYPES.REMOTE_SHARE) &&
          (this.config.experimental.shareReceiverTransform?.worker)) {
        const transceiver = this.mediaProperties.webrtcMediaConnection.multistreamConnection.pc.getTransceivers()
          .find((tr) => (tr.receiver.track.id === event.track.id));
        ...
        this.config.experimental.shareReceiverTransform.worker.postMessage({
          operation: 'decode share',
          readable,
          writable
        }, [readable, writable]);

        console.log('marcin: setting up shareReceiverTransform for remote share');
        shareTransformSetupDone = true;
      }
    });
    ...
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.CONNECTION_STATE_CHANGED, (event) => {
      ...
      LoggerProxy.logger.info(`Meeting:index#setupMediaConnectionCallbacks --> connection state changed to ${event.state}`);
      switch (event.state) {
        ...
        case MC.ConnectionState.DISCONNECTED:
          ...
          this.reconnectionManager.waitForIceReconnect()
            .catch(() => {
              LoggerProxy.logger.info('PeerConnectionManager:index#setupMediaConnectionCallbacks --> state DISCONNECTED, automatic reconnection timed out.');
    ...
    this.mediaProperties.webrtcMediaConnection.on(MC.Event.REMOTE_VIDEO_SOURCE_CHANGED, (data) => {
      // const members = this.members.membersCollection.getAll();

      console.log(`marcin3: VIDEO SOURCE CHANGED: data=${JSON.stringify(data)}`);
      // console.log('marcin3: members=', members);

      const {idx, state, csi} = data;

      this.mediaProperties.remoteVideoSlots[idx].state = state;

      // todo: refactor this - make it a function inside mediaProperties
      if (csi) {
        const member = this.members.membersCollection.findMemberByCsi(csi);

        this.mediaProperties.remoteVideoSlots[idx].csi = csi;
        this.mediaProperties.remoteVideoSlots[idx].memberId = member?.id;
      }
      else {
        this.mediaProperties.remoteVideoSlots[idx].csi = undefined;
        this.mediaProperties.remoteVideoSlots[idx].memberId = undefined;
      }

      console.log('marcin3: remoteVideoSlots=', this.mediaProperties.remoteVideoSlots);
      Trigger.trigger(
        this,
        {
          file: 'meeting/index',
          function: 'setupMediaConnectionCallbacks:Event.REMOTE_VIDEO_SOURCE_CHANGED'
        },
        EVENT_TRIGGERS.MEDIA_REMOTE_VIDEO_SOURCES_CHANGED,
        {
          remoteVideoSlots: this.mediaProperties.remoteVideoSlots
        }
      );
    });

    this.mediaProperties.webrtcMediaConnection.on(MC.Event.ACTIVE_SPEAKERS_CHANGED, ({csis}) => {
      const memberIds = csis.map((csi) => this.members.membersCollection.findMemberByCsi(csi)?.id);

      this.mediaProperties.activeSpeakerMemberIds = memberIds;

      Trigger.trigger(
        this,
        {
          file: 'meeting/index',
          function: 'setupMediaConnectionCallbacks:Event.ACTIVE_SPEAKERS_CHANGED'
        },
        EVENT_TRIGGERS.MEDIA_ACTIVE_SPEAKERS_CHANGED,
        {
          memberIds
        }
      );
  ...
  addMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      .then(() => {
        ...
        const mc = Media.attachMedia(this.mediaProperties, this.mediaId, this.correlationId, this.roap, {
          meetingId: this.id,
        ...
        this.setupMediaConnectionCallbacks();

        return this.mediaProperties.webrtcMediaConnection.initiateOffer();
      })
      .then(() => {
        this.setMercuryListener();
        // PeerConnectionManager.setPeerConnectionEvents(this); // todo
      })
      ...
      .then(() => {
        ...
        // MeetingUtil.startInternalStats(this); // todo

        if (this.config.metrics.autoSendMQA) {
          // this.startMediaQualityMetrics(); // todo
        }
      ...
      .then(() => {
        ...
        return new Promise((resolve, reject) => {
          ...
          console.log(`marcin: webrtcMediaConnection.getConnectionState(): ${connState}`);
  ...
  updateMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      ...
      .then(() => Media.updateMedia(this.mediaProperties, {
        ...
        .then(() => {
          LoggerProxy.logger.info(`${LOG_HEADER} Media.updateMedia done`);
        })
        .catch((error) => {
        ...
        // if not, ensure it's called after
  ...
  updateAudio(options) {
    ...
    return MeetingUtil.validateOptions({sendAudio, localStream: stream})
      ...
      .then(() => {
        ...
        // todo: maybe this.mediaProperties.mediaDirection could be removed? it's duplicating stuff from webrtc-core
  ...
  updateShare(options) {
    ...
    return MeetingUtil.validateOptions({sendShare, localShare: stream})
      ...
      .finally(() => {
        ...
        // const sharingModeIsActive = this.mediaProperties.peerConnection.shareTransceiver.direction === SENDRECV;  // todo
        const isSharingOutOfSync = false; // sharingModeIsActive && !this.isLocalShareLive; // todo

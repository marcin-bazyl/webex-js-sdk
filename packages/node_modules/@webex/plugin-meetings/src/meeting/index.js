...
import {MeetingNotActiveError, createMeetingsError, UserInLobbyError, NoMediaEstablishedYetError, UserNotJoinedError, InvalidSdpError} from '../common/errors/webex-errors';
 ...
 * @memberof Meeting
 */
// todo: unmuted event (remove payload for the event)
/**
...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  setUpLocusInfoSelfListener() {
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_LOCAL_UNMUTE_REQUIRED, () => {
      if (this.audio) {
        this.audio.handleServerLocalUnmuteRequired();
        Trigger.trigger(
          this,
          {
            file: 'meeting/index',
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_REMOTE_MUTE_STATUS_UPDATED, (payload) => {
      if (payload) {
        console.log('marcin: SELF_REMOTE_MUTE_STATUS_UPDATED: with payload=', payload);
        if (this.audio) {
        ...
        Trigger.trigger(
          ...
          payload.muted ? {unmuteAllowed: payload.unmuteAllowed} : {}
        );
      }
    });
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.MEDIA_STATUS_CHANGE, (status) => {
      console.log('marcin: LOCAL_MUTE_EVENT: with status=', status);
   ...
   * @memberof Meeting
   */
  mute(memberId, mute = true, requestToUnmute = undefined) {
    return this.members.muteMember(memberId, mute, requestToUnmute);
  }
  ...
  muteAudio() {
    ...
    console.log('marcin: STARTING audio MUTE idx=', idxCopy);

    // First, stop sending the local audio media
    return logRequest(this.audio.handleClientRequest(true)
      .then(() => {
        console.log('marcin: FINISHED audio MUTE idx=', idxCopy);
        Metrics.postEvent({
  ...
  unmuteAudio() {
    ...
    console.log('marcin: STARTING audio UNMUTE idx=', idxCopy);

    // First, send the control to unmute the participant on the server
    return logRequest(this.audio.handleClientRequest(false)
      .then(() => {
        console.log('marcin: FINISHED audio UNMUTE idx=', idxCopy);
        Metrics.postEvent({
  ...
  muteVideo() {
    ...
    return logRequest(this.video.handleClientRequest(true)
      .then(() => {
        Metrics.postEvent({
          event: eventType.MUTED,
          meeting: this,
  ...
  unmuteVideo() {
    ...
    return logRequest(this.video.handleClientRequest(false)
      .then(() => {
        Metrics.postEvent({
          event: eventType.UNMUTED,
          meeting: this,
  ...
  preMedia(localStream, localShare, mediaSettings) {
    ...
    // the tracks have changed, so we need to re-apply the current mute state to them
    if (this.audio) this.audio.applyClientStateLocally();
    if (this.video) this.video.applyClientStateLocally();
  }

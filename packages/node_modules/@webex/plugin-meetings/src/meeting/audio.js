...
const handleTransition = (audio) => {
  console.log('marcin: handleTransition idx=', audio.idx, ' audio.mute=', audio.mute ? 'yes' : 'no');
  ...
  console.log('marcin: returning null as new state');
    ...
    .then((locus) => {
      LoggerProxy.logger.log(
        `Meeting:audio#doToggle --> AudioStateMachine->onAfterToggle#${transition.event} fired! State changed from '${transition.from}' to '${
          transition.to
        }' with transition '${transition.transition}''.` // this log is misleading because state is not changed at this point
      );
    ...
    return new StateMachine({
      transitions: [
        {
           ...
           */
          to(audio) {
            const newState = handleTransition(audio) || this.state;

            console.log('marcin: newState=', newState);
      ...
      methods: {
        ...
        onBeforeToggle(transition, audio) {
          console.log('marcin: idx=', audio.idx, ' from', transition.from, ' to', transition.to);
          // console.log('marcin: handleTransition audio.mute=', audio.mute ? "yes" : "no");

...
class MuteState {
  ...
  userMuteRequest(mute) {
    ...
    return this.syncDesiredStateWithServer();
  }
  ...
  isDesiredStateMatchingServerState() {
    LoggerProxy.logger.debug(`Meeting:muteState#isDesiredStateMatchingServerState --> ${this.type}: comparing local mute desired state ${this.state.userDesiredState.localMute} with server state: ${this.state.currentServerState.localMute}`);
    LoggerProxy.logger.debug(`Meeting:muteState#isDesiredStateMatchingServerState --> ${this.type}: comparing remote mute desired state ${this.state.userDesiredState.remoteMute} with server state: ${this.state.currentServerState.remoteMute}`);
  ...
  syncDesiredStateWithServer() {
    ...
    if (isLocalMuteMatching && isRemoteMuteMatching) {
      LoggerProxy.logger.debug(`Meeting:muteState#syncDesiredStateWithServer --> ${this.type}: desired state already matching server state, nothing to do`);

      return Promise.resolve();
    }

    if (this.state.requestInProgress) {
      LoggerProxy.logger.debug(`Meeting:muteState#syncDesiredStateWithServer --> ${this.type}: request in progress, we need to wait for it to complete`);

      // todo: we shouldn't resolve now, but later when we finished all the requests
      return Promise.resolve();
    }
    ...
    return localMuteSyncPromise
      ...
      .then(() => {
        ...
        return this.syncDesiredStateWithServer(); // todo: this is recursion, do we need to worry about that?
      });
  }
  ...
  sendLocalMuteRequestToServer() {
    ...
    return MeetingUtil.remoteUpdateAudioVideo(audioMuted, videoMuted, this.meeting)
      .then((locus) => {
        ...
        // todo: temp hack, really I think we should parse the locus reply and make sure that onFullLocus ends up calling handleServerLocalMuteUpdate
  ...
  handleServerRemoteMuteUpdate(muted) {
    if (muted === this.state.currentServerState.remoteMute) {
      LoggerProxy.logger.debug(`Meeting:muteState#handleServerRemoteMuteUpdate --> ${this.type}: currentServerState.remoteMute already matchin server state (${muted})`);
    }

    LoggerProxy.logger.debug(`Meeting:muteState#handleServerRemoteMuteUpdate --> ${this.type}: updating server remoteMute to (${muted})`);
  ...
  isMuted() {
    return this.state.userDesiredState.localMute;
  }

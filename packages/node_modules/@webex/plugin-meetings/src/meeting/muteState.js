...
class MuteState {
  ...
  handleClientRequest(mute) {
    if (mute === this.state.client.localMute) {
      LoggerProxy.logger.info(`Meeting:muteState#handleClientRequest --> ${this.type}: ignoring user request as it is matching existing client state (mute=${mute})`);
